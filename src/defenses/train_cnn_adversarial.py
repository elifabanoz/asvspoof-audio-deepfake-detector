...
def train_one_epoch_adversarial(model, dataloader, criterion, optimizer, device, epsilon):
    model.train()
...
def save_training_curves(train_losses, val_losses, train_accs, val_accs):
    ...
    loss_path = FIGURES_DIR / "adv_cnn_training_loss.png"
    acc_path = FIGURES_DIR / "adv_cnn_validation_accuracy.png"
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Adversarially Trained CNN Loss")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Adversarially Trained CNN Validation Accuracy")
    plt.legend()
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("Training Adversarially Robust CNN")
    print("=" * 60)
    ...
    epochs = 20
    adversarial_epsilon = 0.003

    print(f"Adversarial training epsilon: {adversarial_epsilon}")
    print()
    ...
    ADV_CNN_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), ADV_CNN_MODEL_PATH)

    METRICS_DIR.mkdir(parents=True, exist_ok=True)

    metrics = {
        "model": "Adversarially Trained Spectrogram CNN",
        "feature_type": "mel_spectrogram",
        "defense": "FGSM adversarial training",
        "adversarial_training_epsilon": adversarial_epsilon,
        "epochs": epochs,
    ...
    metrics_path = METRICS_DIR / "adv_cnn_metrics.json"
    ...
    print(f"Model saved to: {ADV_CNN_MODEL_PATH}")
