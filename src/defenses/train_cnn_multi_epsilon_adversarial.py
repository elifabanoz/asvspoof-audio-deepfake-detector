...
def create_fgsm_examples(model, X_batch, y_batch, epsilon, criterion):
...
def train_one_epoch_multi_epsilon(model, dataloader, criterion, optimizer, device, epsilon_values):
    model.train()
    ...
    return avg_loss, accuracy, used_epsilons
...
def save_training_curves(train_losses, val_losses, train_accs, val_accs):
    ...
    loss_path = FIGURES_DIR / "multi_eps_adv_cnn_training_loss.png"
    acc_path = FIGURES_DIR / "multi_eps_adv_cnn_validation_accuracy.png"
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Multi-Epsilon Adversarial CNN Loss")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Multi-Epsilon Adversarial CNN Validation Accuracy")
    plt.legend()
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("Training Multi-Epsilon Adversarially Robust CNN")
    print("=" * 60)
    ...
    epochs = 20
    epsilon_values = [0.001, 0.003, 0.005, 0.010]

    print(f"Multi-epsilon adversarial training values: {epsilon_values}")
    print()
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_acc, used_epsilons = train_one_epoch_multi_epsilon(
            model=model,
        ...
        epsilon_counts = {
            str(eps): used_epsilons.count(eps)
            for eps in epsilon_values
        }
        epoch_epsilon_usage[str(epoch)] = epsilon_counts
    ...
    MULTI_EPS_ADV_CNN_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), MULTI_EPS_ADV_CNN_MODEL_PATH)

    METRICS_DIR.mkdir(parents=True, exist_ok=True)

    metrics = {
        "model": "Multi-Epsilon Adversarially Trained Spectrogram CNN",
        "feature_type": "mel_spectrogram",
        "defense": "multi-epsilon FGSM adversarial training",
        "adversarial_training_epsilons": epsilon_values,
        "epochs": epochs,
        ...
        "epoch_epsilon_usage": epoch_epsilon_usage,
    }

    metrics_path = METRICS_DIR / "multi_eps_adv_cnn_metrics.json"
    ...
    print()
    print("Multi-epsilon adversarial training completed successfully.")
    print(f"Model saved to: {MULTI_EPS_ADV_CNN_MODEL_PATH}")
